    ...
    const fetchInquiries = async () => {
      ...
      try {
        const response = await axios.get(
          `${API_HOST}/inquiry/customer/inquiries`,
          {
        ...
        const mockData = [
          {
            ...
            messages: [
              {
                senderId: "cust_1",
                text: "Can you convert this figma to next Js?",
                createdAt: "2026-02-23T21:23:00.000Z",
              },
              {
                senderId: "seller_1",
                text: "Yea sure!",
                createdAt: "2026-02-23T22:02:00.000Z",
              },
              {
                senderId: "cust_1",
                text: "Thanks a lot❤",
                createdAt: "2026-02-23T22:05:00.000Z",
              },
            ],
          },
          {
            ...
            messages: [
              {
                senderId: "cust_1",
                text: "I need help in this html page...",
                createdAt: "2026-02-22T14:23:00.000Z",
              },
            ],
          },
          {
            ...
            messages: [
              {
                senderId: "cust_1",
                text: "What about my project?",
                createdAt: "2026-02-20T09:00:00.000Z",
              },
              {
                senderId: "seller_3",
                text: "It is completed and delivered.",
            ...
            <div className="w-2/3 flex flex-col bg-white">
              {selectedInquiry ? (
                <>
                  ...
                  <div className="flex-1 overflow-y-auto p-8 bg-white flex flex-col">
                    ...
                    <div className="bg-[#e29525] rounded-xl p-4 text-white max-w-sm self-center w-full mb-8 shadow-sm">
                      <div className="text-xs opacity-90 mb-1">
                        Inquiry #{selectedInquiry.uiStaticId}
                      </div>
                      <div className="font-bold text-lg leading-tight mb-1">
                        {selectedInquiry.item.title}
                      </div>
                      <div className="font-bold mb-1">
                        Price: $ {selectedInquiry.item.price}
                      </div>

                      {/* CONDITIONAL LOGIC: Disable link if status is closed */}
                      {selectedInquiry.status.toLowerCase() !== "closed" ? (
                        <Link
                          to={`/details/${selectedInquiry.itemType}/${selectedInquiry.item._id}`}
                          className="text-blue-700 font-medium text-sm hover:underline block mb-4"
                        >
                          [Place Order]
                        </Link>
                      ) : (
                        <span className="text-gray-200 font-medium text-sm block mb-4 cursor-not-allowed">
                          [Place Order]
                        </span>
                      )}
                    ...
                    <div className="space-y-6 flex-1 flex flex-col">
                      {selectedInquiry.messages.map((msg, idx) => {
                        ...
                        const isCustomer =
                          msg.senderId === selectedInquiry.customerId._id;

                        return (
                          <div
                            key={idx}
                            className={`flex flex-col max-w-[70%] ${isCustomer ? "self-end" : "self-start"}`}
                          >
                            <div
                            ...
                            <span
                              className={`text-[10px] text-gray-400 mt-1 italic ${isCustomer ? "text-right" : "text-left"}`}
                            >
                  ...
                  {/* CONDITIONAL INPUT AREA: Shows Input OR "Closed" text */}
                  {selectedInquiry.status.toLowerCase() === "closed" ? (
