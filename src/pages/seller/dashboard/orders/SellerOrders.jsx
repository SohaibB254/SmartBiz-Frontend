      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-137.5 p-8 relative border border-gray-100">
        ...
        <div className="text-sm text-black space-y-6">
          <div>
            ...
            <p>Item/Service: <span className="font-semibold text-[#e29525]">{order.item.title}</span></p>
            <div className="mt-1 leading-relaxed">
              <p>Timeline:</p>
              <p className="text-gray-600">. Order Placed: {order.date_placed.split('T')[0] || '22-02-2026'}</p>
              <p className="text-gray-600">. Approved: 22-02-2026</p>
              <p className="text-gray-400">. Completed: {order.status === 'completed' ? order.date_completion.split('T')[0] : 'Pending...'}</p>
            </div>
            <div className="mt-3 flex items-center">
              <span className="mr-2">Order status:</span>
              <span className={`px-3 py-0.5 rounded-full text-white text-xs font-medium ${order.status === 'pending' ? 'bg-[#e2c525]' : order.status === 'Completed' ? 'bg-green-500' : 'bg-red-500'}`}>
                {order.status}
              </span>
            </div>
          </div>
          ...
          <div>
            ...
            <p className="mb-1">Total Amount: <span className="text-[#e29525] font-semibold">${order.amount}</span></p>
            <p className="mb-1 flex items-center">
              Payment status: <span className="ml-2 px-2 py-0.5 rounded-full text-white text-xs font-medium bg-green-500">Paid</span>
            </p>
          ...
          <div className="relative">
            ...
            <p className="mb-1">Email : <a href="#" className="text-blue-600 hover:underline">{order.customerId.email}</a></p>

            {/* Action Buttons (Only visible if pending) */}
            {order.status === 'pending' && (
              <div className="absolute bottom-0 right-0 flex space-x-3">
                <button onClick={() => handleAction('completed')} disabled={isProcessing} className="bg-[#1ecb4f] hover:bg-[#19b043] text-white text-sm font-medium py-2 px-6 rounded-full transition duration-200 shadow-sm disabled:opacity-70">
                  {isProcessing ? 'Processing...' : 'Mark as Completed'}
                </button>
                <button onClick={() => handleAction('cancelled')} disabled={isProcessing} className="bg-[#e04524] hover:bg-[#c93a1c] text-white text-sm font-medium py-2 px-6 rounded-full transition duration-200 shadow-sm disabled:opacity-70">
                  Cancel
                </button>
              </div>
            )}
...
const SellerOrders = () => {
  const { orders, isLoading, fetchOrders, updateOrderStatus } = useSellerOrder();
  ...
  const handleStatusUpdate = async (customOrderId, action) => {
    try {
      ...
      setToastConfig({ isVisible: true, success: false, message: "Failed to update order status." });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      <header className="fixed top-0 w-full z-40 h-18.5 border-b border-gray-200 bg-white px-6 py-4 flex justify-between items-center">
        ...
        <main className="flex-1 p-8  min-h-[calc(100vh-73px)]">
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex items-center space-x-6 text-sm font-medium">
              <div className="text-gray-700">{businessProfile?.title || 'Business'}</div>
          ...
          <div className="w-full">
            <div className="grid grid-cols-7 gap-4 px-6 py-3 font-semibold text-sm text-gray-900 border-b border-gray-200">
              ...
              <div className="col-span-1 flex items-center">Order Date <ChevronDown className="w-4 h-4 ml-1" /></div>

              <div className="col-span-1 relative">
                <div className="flex items-center cursor-pointer" onClick={() => setIsFilterDropdownOpen(!isFilterDropdownOpen)}>
                ...
                {isFilterDropdownOpen && (
                  <div className="absolute top-full left-0 mt-1 w-32 bg-white border border-gray-200 shadow-lg rounded-md overflow-hidden z-10">
                    {['All', 'Pending', 'Completed', 'Cancelled'].map(status => (
                      <div key={status} onClick={() => { setStatusFilter(status); setIsFilterDropdownOpen(false); }} className="px-4 py-2 text-sm cursor-pointer hover:bg-gray-50">{status}</div>
                    ))}
                  </div>
                )}
              </div>
            ...
            <div className="mt-2 space-y-3">
              {isLoading ? (
                <div className="flex justify-center py-20"><Loader2 className="w-8 h-8 animate-spin text-[#e29525]" /></div>
              ) : orders.map((order) => (
                <div key={order.customOrderId} className="grid grid-cols-7 gap-4 px-6 py-4 items-center bg-white border border-gray-200 rounded-lg text-sm">
                  <div className="col-span-1 text-blue-600">{order.customOrderId}</div>
                  <div className="col-span-2 font-medium">{order.item.title}</div>
                  <div className="col-span-1 text-gray-700">{order.customerId.username}</div>
                  ...
                  <div className="col-span-1 flex items-center">
                    ...
                    <span className={order.status === 'pending' ? 'text-[#e2c525]' : order.status === 'completed' ? 'text-green-500' : 'text-red-500'}>{order.status}</span>
                  </div>
                  <div className="col-span-1 flex justify-between items-center">
                    <span className="font-medium">$ {order.amount}</span>
                    <button onClick={() => setSelectedOrder(order)} className="text-blue-600 font-medium px-2 py-1 hover:underline">View</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      <SellerOrderModal isOpen={!!selectedOrder} onClose={() => setSelectedOrder(null)} order={selectedOrder} onStatusUpdate={handleStatusUpdate} />
    </div>
  );
};
