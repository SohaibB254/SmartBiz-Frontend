  ...
  useEffect(() => {
    const fetchItem = async () => {
      try {
        ...
        const fetchedItem = data.item || data; // Safely handle whether it's wrapped in 'item' or not

        setItem(fetchedItem);

        // Pre-fill form state correctly using the extracted fetchedItem
        setFormData({
        ...
        // Safely set image preview to avoid "http://localhost:3000/undefined"
        setImagePreview(fetchedItem.image ? `${API_HOST}/${fetchedItem.image}` : (fetchedItem.imageUrl || ""));
      } catch (error) {
        setToastConfig({
          isVisible: true,
          success: false,
        ...
        setItem(mockItem);
        // Also pre-fill form data for the mock!
        setFormData({
          ...
          <div className="w-full max-w-3xl bg-white border border-gray-100 rounded-3xl p-8 shadow-sm">
            ...
            <div className="w-full h-87.5 bg-blue-100 rounded-3xl overflow-hidden mb-8 relative">
              <img
                src={
                  imagePreview ||
                  "https://placehold.co/800x400/e2e8f0/64748b?text=Item"
                }
                alt="Listing"
            ...
            {!isEditing ? (
              <div>
                ...
                <div className="mb-10">
                  ...
                  <p className="text-gray-500 whitespace-pre-line leading-relaxed max-w-2xl">
                    {item.description}
                  </p>
                </div>
                ...
                <div className="flex space-x-4">
                  <button
                    onClick={() => {
                      // Bulletproof fix: Sync form data perfectly right before switching modes
                      setFormData({
                        title: item.title,
                        price: item.price,
              ...
              <form onSubmit={handleUpdate}>
                ...
                <div className="flex space-x-4">
                  ...
                  <button
                    ...
                    onClick={() => {
                      setIsEditing(false);
                      setImagePreview(item.image ? `${API_HOST}/${item.image}` : (item.imageUrl || ""));
                    }}
